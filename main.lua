...
local function AutoCanvas(scrolling, layout, horizontal)
	local function update()
		local pad = 16
		if horizontal then
			local x = layout.AbsoluteContentSize.X + pad
			Tween(scrolling, {CanvasSize = UDim2.new(0, x, 0, 0)}, 0.18)
		else
...
function UI:CreateTab(opt)
	...
	local tabBar = self._Main:FindFirstChild("Body") and self._Main:FindFirstChild("Body"):FindFirstChild("TabBar")
	local tabList = tabBar and tabBar:FindFirstChild("Tabs")
	...
	if tabList then
		...
		surface.Size = UDim2.fromOffset(120, 32)
		surface.ZIndex = 32
		AddCorner(surface, 8)
		...
		textLbl.Position = UDim2.fromOffset(icon and 28 or 8, 0)
